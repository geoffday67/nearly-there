...
var output = fs.createWriteStream('./stations.dat');

rl.createInterface(fs.createReadStream('./stations.csv'))
.on('line', (line) => {
    let parts = line.split(',');
    let name = parts[0];
    let easting = parts[1];
    let northing = parts[2];

    osgb.setGridCoordinates(easting, northing);
    let wgs84 = osgb.getWGS84();
    output.write(`${name},${wgs84.latitude},${wgs84.longitude}\n`);
});
